...
our %config = (
    ...
    "openssl_sys_defines" => [],
    "openssldir" => "/usr/local/openssl-x86/ssl",
    "options" => "--prefix=/usr/local/openssl-x86 --openssldir=/usr/local/openssl-x86/ssl no-acvp-tests no-asan no-brotli no-brotli-dynamic no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-demos no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fips no-fips-jitter no-fips-post no-fips-securitychecks no-fuzz-afl no-fuzz-libfuzzer no-h3demo no-hqinterop no-jitter no-ktls no-loadereng no-md2 no-msan no-pie no-rc5 no-sctp no-shared no-ssl3 no-ssl3-method no-sslkeylog no-tests no-tfo no-trace no-ubsan no-unit-test no-uplink no-weak-ssl-ciphers no-winstore no-zlib no-zlib-dynamic no-zstd no-zstd-dynamic",
    "patch" => "4",
...
unless (caller) {
    ...
    use lib '/home/cyg/mqtt_x86/mqtt.client/openssl/openssl-3.5.4/util/perl';
    use OpenSSL::fallback '/home/cyg/mqtt_x86/mqtt.client/openssl/openssl-3.5.4/external/perl/MODULES.txt';
    ...
    if (scalar @ARGV == 0) {
        ...
        use lib '.';
        use lib '/home/cyg/mqtt_x86/mqtt.client/openssl/openssl-3.5.4/Configurations';
        use gentemplate;
